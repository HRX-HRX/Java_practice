...
/*import com.example.demo.modl.ArticleInfo;*/
...
public class UserController {
    ...
    // @Autowired报错的原因： @Autowired需要一个非空的对象，而@Autowired引入的对象却是 Mybatis的 @Mapper 注解，
    // 而idea 不能很好地识别 mybatis的注解，所以idea 不能确认当前要注入的对象是否为非null对象
    //解决方案：
    //1.给 Mapper 添加一个 idea 可以识别的 Spring 注解，例如 @Repository、@Component
    //2.直接把idea 的注解关掉
...
//    private Logger logger = LoggerFactory.getLogger(UserController.class);
    ...
    @ResponseBody // 当前方法返回的为数据而非视图
    public Object login0(User user, HttpServletRequest request){
        // 返回的对象
